...
const mockEmployees = [
  { id: 'emp-1', name: 'Employee 1', businessInfoId: 'business-1' },
  { id: 'emp-2', name: 'Employee 2', businessInfoId: 'business-1' },
];
...
function generateBaseSlots(
  ...
  const maxTime = (date: moment.Moment) =>
    date.clone().set({ hour: maxHour, minute: maxMinute, second: 0, millisecond: 0 });

  // Iterate through each day
  for (let dayOffset = 0; dayOffset < days; dayOffset++) {
    const currentDate = moment.utc(startDate).add(dayOffset, 'days').startOf('day');
    ...
    if (dayOffset === 0 && clientCurrentTime) {
      ...
      if (clientCurrentDateStr === dateISO) {
        ...
        const currentTimeMoment = currentDate
          .clone()
          .set({ hour: effectiveMinHour, minute: effectiveMinMinute, second: 0, millisecond: 0 });
        ...
        if (finalMinTime.isAfter(maxTimeForDay) || finalMinTime.isSame(maxTimeForDay)) {
          console.log(`Skipping ${dateISO} - current time (${effectiveMinHour}:${effectiveMinMinute.toString().padStart(2, '0')}) is at or past maxHour (${maxHour}:${maxMinute.toString().padStart(2, '0')})`);
          continue;
        }
      } else {
        console.log(`Skipping ${dateISO} - client current time is on different day (${clientCurrentDateStr})`);
        continue;
      }
    } else {
      effectiveMinHour = minHour;
...
const slots1 = generateBaseSlots(startDate1, 1, mockService.measure + config.splitTime, config, 0, today10AM);
console.log(`Generated ${slots1.length} slots for today starting at 10:00 AM`);
slots1.slice(0, 5).forEach((slot) => {
  console.log(`  ${slot.start.format('YYYY-MM-DD HH:mm')} - ${slot.end.format('HH:mm')}`);
});
...
const slots2 = generateBaseSlots(startDate2, 1, mockService.measure + config.splitTime, config, 0, today7PM);
...
const slots3 = generateBaseSlots(startDate3, 1, mockService.measure + config.splitTime, config, 0);
...
const slots4 = generateBaseSlots(startDate4, 1, mockService.measure + config.splitTime, config, 0, today830AM);
console.log(`Generated ${slots4.length} slots for today starting at 8:30 AM`);
if (slots4.length > 0) {
  console.log(`  First slot: ${slots4[0].start.format('YYYY-MM-DD HH:mm')} - ${slots4[0].end.format('HH:mm')}`);
  if (slots4[0].start.hour() === 8 && slots4[0].start.minute() === 30) {
    console.log('  ✓ Correctly starts at 8:30');
  } else {
    console.log(`  ✗ Should start at 8:30, but starts at ${slots4[0].start.format('HH:mm')}`);
  }
}
...
if (slots5.length > 0) {
  console.log(`  First slot: ${slots5[0].start.format('YYYY-MM-DD HH:mm')} - ${slots5[0].end.format('HH:mm')}`);
  if (slots5[0].start.hour() === 8 && slots5[0].start.minute() === 0) {
    console.log('  ✓ Correctly rounded up to 8:00');
  } else {
    console.log(`  ✗ Should round up to 8:00, but starts at ${slots5[0].start.format('HH:mm')}`);
  }
} else {
